...
class Grid(object):
    ...
    # Snap to the nearest point on the grid
    ...
    def nearby(self, x, y):
        # Is there an existing point or line nearby?
        points = []
        lines = []
    ...
    def insert(self, x, y, line=None):
        # Insert if it isn't too close to another point
...
def new_angle(a, d):
    # For the first iteration, spread out in a circle
    if d < 0.1:
        return random.random() * 2 * pi

    # For subsequent iterations, pick an angle based around the current angle with a standard deviation of pi / 10
    else:
...
def make_path(pairs):
    # Do a depth first traversal of the pairs
