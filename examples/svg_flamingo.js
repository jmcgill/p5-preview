...
function drawPath(path) {
  ...
  for (let component of components) {
    ...
    if (component[0] === 'M') {
      ...
      // console.log('Vertex at ', x, y, x0, y0, x0 + x, y0 + y);
      ...
      // console.log('Line to', x + x1, y + y1, p[0], p[1], component);
      reflectedControl = [x, y];
    } else if (component[0] === 'L') {
      let m = component.substr(1).replace(/-/g, ',-');
      if (m[0] === ',') {
...
function setup() {
  ...
  const data = fs.readFileSync('/Users/jimmy/Desktop/morph_4.svg');
  ...
  //translate(162, 290);
  textSize(1);
  text("Transformations #1", 0, 0);
  pop();

  const n = 120;
  translate(59 + 5, 80);
  scale(25.4 / n, (25.4 / n) * 1.00795);
