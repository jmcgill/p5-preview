...
function drawPath(path) {
    ...
    if (match) {
        // console.log('TRANSLATING', parseFloat(match[1]), parseFloat(match[2]));
        // scale(1, -1);
        // 0.002756 is hard coded from the scale command on SVG - bring it in
    ...
    const matchScale = /scale\(([-.\d]+), ([-.\d]+)\)/.exec(transform);
    if (matchScale) {
        console.log('SCALING', parseFloat(matchScale[1]), parseFloat(matchScale[2]));
        // scale(1, -1);
        // 0.002756 is hard coded from the scale command on SVG - bring it in
        scale(parseFloat(matchScale[1]), parseFloat(matchScale[2]));
        // scale(1, -1);
        // x = parseFloat(match[1]) + 50;
        // y = parseFloat(match[2]);
    }
...
async function setup() {
    ...
    font2.drawGlyph('N', 20);
    pop();

    // const font = await window.electronApi.readFile('/Users/jmcgill/test-font.svg');
    // const parser = new DOMParser();
    // const f = parser.parseFromString(font, "text/xml");
    // const g = f.getElementById("g1914");
    // console.log(g);
    // drawGroup(g, {}, '');
    //drawPath(g);
