...
Plotter.prototype._processQueue = function() {
  ...
  if (!this.connected && this._outputFile) {
    ...
    console.log('RS232: Sending read buffer bytes');
    this.send(this.RS232_PREFIX + "B", (data) => {
      console.log('RS232: Got response: ', data);
      ...
      console.log('RS232 Checking Queue');
      if (this._queue[0] && this._queue[0].instruction.length <= freeSpace) {
        console.log('Queue 0 instruction length is: ', this._queue[0].instruction.length, freeSpace);
